const LanguageService = {
  ...
  async onCorrectGuess(db, word_id, user_id) {
    return db
      .raw(`
            ...
            UPDATE word SET memory_value = memory_value * 2 WHERE id = ${word_id};
 ...
 async onWrongGuess(db, word_id, user_id) {
    return db
      .raw(`
        UPDATE word SET incorrect_count = incorrect_count + 1 WHERE id = ${word_id};
        UPDATE word SET memory_value = 1 WHERE id = ${word_id};
      `)
      .then(async () => {
        const updatedLang = await this.getUsersLanguage(db, user_id)
        const updatedWord = await this.getWord(db, word_id);
